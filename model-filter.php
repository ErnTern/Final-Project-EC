...
function selectClass() {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT champ_id, champ_name, class_id, lane_id FROM `Class`");
...
function selectClassByChampion($iid) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT champ_name, class_id, lane_id, champ_id FROM Champion WHERE class_id = ?");
